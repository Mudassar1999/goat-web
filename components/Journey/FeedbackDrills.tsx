...
const FeedbackDrills = () => {
  ...
  const [isDrillDetailsDialogOpen, setIsDrillDetailsDialogOpen] =
    useState<any>(false);
  useEffect(() => {
    fetchFeedbacks();
  }, []);
  ...
  const saveFeedback = (e: any) => {
    ...
    axios
      .post(
        `${config.URL}/journies/userDrill/${selectedDrillId}/feedback/admin`,
        data,
        {
          headers: {
    ...
    <div className="fixed">
              ...
              {brandData1.map((brand: any, index: any) => (
                <Card key={index} className="mb-4">
                  ...
                  >
                    ...
                    >{`  ${brand.User.firstName} ${brand.User.lastName}`}</b>
                    <label
                      style={{ fontSize: "small" }}
                    >{` (${brand.User.userName})`}</label>
                  </span>
                  <p
                    style={{
                      marginLeft: "20px",
      ...
      <Dialog open={isDialogOpen} onClose={closeDialog}>
        ...
        <Dialog
          ...
          onClose={closeDrillDetailsDialog}
        >
            ...
            {selectedDrillData && (
              <>
                ...
                <div className="p-1 xl:p-0">
                  {selectedDrillData.objectives &&
                  selectedDrillData.objectives.length > 0 ? (
                    selectedDrillData.objectives.map(
                      (objective: any, objectiveIndex: any) => (
                        <div key={objective.id}>
                          ...
                          {objectiveIndex !==
                            selectedDrillData.objectives.length - 1 && (
                            <span></span>
                          )}
                        </div>
                      )
